...
export default {
  name: "esForm",
  emits: ["change", "submit", "update:modelValue", "inited"],
  /* ====================== 生命周期 ====================== */
  created() {
  ...
  methods: {
    ...
    */
     ...
     * 当表单组件发生改变时：处理事件，同步项
     */
    _syncFormUi(checkSchemas, eventNames, options, mustUpdate) {
      ...
      if (this.isInited) {
        ...
        if (handlers.length > 0 || mustUpdate) {
          // 这个可以记录是什么导致表单改变
    ...
    __syncValue(sourcePathKey) {
      // console.log("sourcePathKey", sourcePathKey);
      // 不单只是执行actions
